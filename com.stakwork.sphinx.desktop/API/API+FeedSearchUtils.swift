...
extension API {
    ...
    public func searchForFeeds(
        ...
        guard let urlString = urlComponents.url?.absoluteString else {
            completionHandler(Result<[FeedSearchResult],API.RequestError>.failure(.failedToCreateRequestURL))
            return
        }
        ...
        guard let request = createRequest(
            ...
            completionHandler(Result<[FeedSearchResult], API.RequestError>.failure(RequestError.failedToCreateRequest(urlPath: urlPath)))
            return
        }
        ...
        podcastSearchRequest = AF.request(request).responseJSON { response in
            ...
            case .success(let data):
                var results : [FeedSearchResult] = [FeedSearchResult]()
